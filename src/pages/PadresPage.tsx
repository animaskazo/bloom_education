...
export default function PadresPage() {
  ...
  async function generarApoderadosPrueba() {
    ...
    try {
      ...
      const { data: students } = await supabase.from('estudiantes').select('id, nombre, apellido').eq('establecimiento_id', perfil.establecimiento_id)
  ...
  async function load() {
    try {
      ...
      const { data: ests } = await supabase.from('estudiantes').select('*, cursos(id, nombre, letra)').eq('establecimiento_id', perfil.establecimiento_id)
      ...
      if (crs && crs.length > 0) setExpandedCursos([crs[0].id])

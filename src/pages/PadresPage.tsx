...
import { Plus, Pencil, Trash2, Users, Phone, Mail, Search, ChevronDown, ChevronUp, UserPlus, X } from 'lucide-react'

type ApoderadoExt = Apoderado & { estudiantes?: { id: string; nombre: string; apellido: string; cursos?: { nombre: string; letra?: string } }[] }
...
export default function PadresPage() {
  ...
  async function load() {
    try {
      setLoading(true)
      const { data: apod } = await supabase.from('apoderados').select('*').order('apellido')
      const { data: rel } = await supabase
        .from('estudiante_apoderado')
        .select('apoderado_id, es_titular, estudiantes(id, nombre, apellido, cursos(nombre, letra))')
      ...
      const { data: ests } = await supabase.from('estudiantes').select('*, cursos(nombre, letra)').eq('estado', 'activo')
      setAvailStudents(ests??[])

      const { data: crs } = await supabase.from('cursos').select('*').eq('estado', 'activo').order('nombre')
      setCursos(crs??[])
    } catch (e: any) {
      console.error(e)
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      <PageHeader
        ...
        action={<button className="btn-primary" onClick={openAdd}><Plus className="w-4 h-4"/>Agregar apoderado</button>}
      />
      ...
      {loading ? (
        ...
        <EmptyState icon={Users} title="Sin apoderados" description="No se encontraron apoderados." action={<button className="btn-primary btn-sm" onClick={openAdd}><Plus className="w-3.5 h-3.5"/>Agregar</button>} />
      ) : (
        <div className="space-y-2">
          {filtered.map(r=>(
            <div key={r.id} className="card overflow-hidden">
              <div className="p-4 flex items-center gap-4 cursor-pointer hover:bg-slate-50 transition-colors" onClick={()=>setExpanded(expanded===r.id?null:r.id)}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-400 font-mono">{r.rut}</p>
                </div>
                <div className="flex items-center gap-2 ml-auto">
                  {(r.estudiantes??[]).length > 0 && (
                    <span className="badge-blue text-xs">{(r.estudiantes??[]).length} pupilo{(r.estudiantes??[]).length>1?'s':''}</span>
                  )}
              ...
              {expanded===r.id && (
                <div className="border-t border-slate-100 bg-slate-50 px-4 py-3 grid grid-cols-2 md:grid-cols-3 gap-3 text-sm animate-fade-in">
                  <Info label="RUT" value={r.rut} mono />
                  <Info label="Teléfono" value={r.telefono} />
                  <Info label="Email" value={r.email} />
                  <div className="col-span-2 md:col-span-3">
                    <div className="flex items-center justify-between mb-2">
                      <p className="text-xs font-semibold text-slate-500">Pupilos a cargo</p>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {(r.estudiantes??[]).length === 0 ? (
                        ...
                        (r.estudiantes??[]).map((e:any,i:number)=>(
                          <div key={i} className="badge-blue pr-1 flex items-center gap-1.5">
